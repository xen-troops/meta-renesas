...
ipl_make_and_move() {
    ...
    install -d ${DEPLOYDIR}/loaders
    # Copy IPL to deploy folder
    # There is no '-' before MEM_SUFFIX because MEM_SUFFIX either is empty or contains '-'
    install -m 0644 ${S}/build/${PLATFORM}/release/bl2/bl2.elf   ${DEPLOYDIR}/loaders/bl2-${MACHINE}${MEM_SUFFIX}.elf
    install -m 0644 ${S}/build/${PLATFORM}/release/bl2.bin       ${DEPLOYDIR}/loaders/bl2-${MACHINE}${MEM_SUFFIX}.bin
    install -m 0644 ${S}/build/${PLATFORM}/release/bl2.srec      ${DEPLOYDIR}/loaders/bl2-${MACHINE}${MEM_SUFFIX}.srec
    install -m 0644 ${S}/build/${PLATFORM}/release/bl31/bl31.elf ${DEPLOYDIR}/loaders/bl31-${MACHINE}${MEM_SUFFIX}.elf
    install -m 0644 ${S}/build/${PLATFORM}/release/bl31.bin      ${DEPLOYDIR}/loaders/bl31-${MACHINE}${MEM_SUFFIX}.bin
    install -m 0644 ${S}/build/${PLATFORM}/release/bl31.srec     ${DEPLOYDIR}/loaders/bl31-${MACHINE}${MEM_SUFFIX}.srec
    if [ -z ${MEM_SUFFIX} ]; then
        # sa0 and sa6 doesn't change for different momory variants, so we build them for default variant only
        install -m 0644 ${S}/tools/renesas/rcar_layout_create/bootparam_sa0.bin  ${DEPLOYDIR}/loaders/bootparam_sa0.bin
        install -m 0644 ${S}/tools/renesas/rcar_layout_create/bootparam_sa0.srec ${DEPLOYDIR}/loaders/bootparam_sa0.srec
        install -m 0644 ${S}/tools/renesas/rcar_layout_create/cert_header_sa6.bin  ${DEPLOYDIR}/loaders/cert_header_sa6.bin
        install -m 0644 ${S}/tools/renesas/rcar_layout_create/cert_header_sa6.srec ${DEPLOYDIR}/loaders/cert_header_sa6.srec
        install -m 0644 ${S}/tools/renesas/rcar_layout_create/cert_header_sa6_emmc.bin  ${DEPLOYDIR}/loaders/cert_header_sa6_emmc.bin
        install -m 0644 ${S}/tools/renesas/rcar_layout_create/cert_header_sa6_emmc.srec ${DEPLOYDIR}/loaders/cert_header_sa6_emmc.srec
    fi
}
